...
Nonetheless, it is still worthwhile considering all the tracers. Indeed a multi tracer-approach can bring the removed power on the overlapping area from 27\% of the CIB alone to 36\% using all the LSS and 41\% when Planck lensing reconstruction is added to the set.
...
\hline
Surveys &$\alpha(r=0), N^{B}_{\ell}=0$& $\alpha(r=0)$ & $\alpha(r=0.12)$ \\ \hline \hline
CMB SPTPol (35\%) &1.46& 1.27  & 1.08 \\ \hline
LSS+CMB (56\%) & 2.18 & 1.57 & 1.16  \\ \hline
...
Despite this, adding galaxy survey will still lead to a decent improvement with a relatively low effort.
...
\begin{table}
\caption{$\alpha$: S3 improvements. The absolute error value of $\sigma(r)$ after delensing for $f_{sky} = $ is $\sigma(r) =$  }
  \begin{tabular}{|c | c | c | c|}
\hline
Surveys & $\alpha(r=0), N^{B}_{\ell}=0$ & $\alpha(r=0)$ & $\alpha(r=0.12)$ \\ \hline \hline
