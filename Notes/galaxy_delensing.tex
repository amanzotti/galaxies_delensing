...
with
\be\label{eqn:rho1}
\rho_{\ell}= \frac{\cl^{\kappa I}}{\sqrt{\cl^{\kappa \kappa} \cl^{I I}}}.
\ee
The bigger the $\rho_{\ell}$ is for an LSS field, the more it is correlated with the lensing potential acting on the CMB photons. An higher correlation allows for a better reconstruction of $\phi$ and, as a consequence, of $B^{\rm{lens}}$.

\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=1.]{../images/BB_res.pdf}
\caption{}
\label{fig:bb_res}
\end{center}
\end{figure}
...
\caption{Tomographic bins improve the cross-correlation with lensing. Here we show the cross-correlation coefficient (\refeq{rho1}) as a function of scale for full surveys (solid lines) and tomographically binned surveys (dot-dashed line).}
\label{fig:bb_res}
\end{center}
\end{figure}
...
In the Fisher Information Matrix formalism \cite{10.2307/2342435}, the statistical uncertainty on a cosmological parameter $p$ can be obtain the from inverse of the Fisher matrix $F_{ij}$ as $\sigma(p) =\sqrt{(\mathbf{F})_{pp}^{-1}}$.
...
We are also neglecting an important contribution to measured CMB B-modes: Galactic polarized foregrounds. 
The amplitude of these has been constrained in \cite{bicep2/keck-collaboration:2015, planck-collaboration:2015} and it strongly varies in different parts of the sky. Future experiments will use multi-frequency data to exploit the frequency dependency of these contaminants to remove them from the data. The amount of residual foregrounds depends both on uncertain foreground properties and experimental choices (see a review in \cite{abazajian:2016}).
For this reason, accurately treating foreground requires the use of simulations and knowledge about experimental details. 
We decided to focus on an ideal situation of no foregrounds or perfect cleaning even if the importance of delensing will be overestimated. 

Furthermore, we are not considering the uncertainties on galaxy survey internal parameters such as biases, source distributions and photometric redshift uncertainties. These parameters do have a significant impact in degrading inflationary constraints \cite{sherwin:2015,namikawa:2016a}.
However, as shown in \cite{sherwin:2015,namikawa:2016a}, these can be auto-calibrated i.e. they can be tightly constrained using galaxy surveys auto spectra and cross-correlation. We checked this for a few of the tracers combinations used here and find it particularly true once several tracers are jointly taken into account.
...
Only recently delensing has been performed on data using both the internal CMB lensing potential reconstruction \cite{carron:2017} as well as CIB maps \cite{larsen:2016a,manzotti:2017} as large scale structure tracers.
...
On the CMB side, we will combine a Bicep-Keck like \textit{deep} CMB experiment with an overlapping \textit{higher resolution} CMB experiment. For the deep experiment we use an instrumental noise equal to \textcolor{red}{$3\mu$K-arcmin}, a beam of \textcolor{red}{30 arcmin} and an angular scale range of $50<\ell<500$.
The CMB lensing reconstruction will be performed by the higher resolution CMB experiment. We explore two possibilities.
First, we test the efficiency with an internal reconstruction performed by Planck \cite{planck-collaboration:2016a} using the CMB lensing noise curves publicly available
...
In these figures we include a dashed curve that corresponds to (with arbitrary scale) $<C_{\ell}^{\kappa \kappa} \times \frac{\partial C^{BB}_{\ell'}}{\partial{C^{\kappa \kappa}_{\ell}}}>_{\ell ' <100}$, showing which scales contribute to the $\ell<100$ B-mode power.
...
On the other end, the CMB lensing reconstruction from SPT-Pol will be more than 80\% correlated with the true field at $\ell<300$, and only then the correlation become comparable with the current LSS surveys.


\reffig{corrnow} shows that the DES is an effective tracer of the LSS and can, at least in the near future, be used to improve delensing for CMB experiments that overlap with it. Indeed its delensing efficiency is higher than WISE because the redshift distributions peaks at slightly higher redshift thus it better overlaps with CMB lensing.
...
For current surveys we only consider $p=r$ since the broad beam of the deep BK-like survey does not provide enough leverage to constrain the shape of the B-mode spectra.
...
\begin{table}
\caption{$\alpha(r)$: Improvements on $\sigma(r)$ due to delensing for current generation and Planck lensing reconstruction. Values correspond to ratio of the error before and after delensing.}
  \vspace{0.2cm}
  \begin{tabular}{ | c | c | c|}
\hline
...
LSS+CMB & 1.71& 1.26  \\ \hline
  ...
  \label{tab:current}
\end{table}
In agreement with the literature \cite{yu:2017,larsen:2016a}, the CIB is the predominant source of delensing with an improvement of almost 50\%. However Planck and DES are essential to reconstruct the largest scales of the lensing potential and as such the play a major role in delensing. The improvement due to WISE is quite marginal, but given the big sky coverage, it might be useful to delens large CMB polarization experiments.
...
An interesting results is that DESI will be less efficient than a DES-like survey even if it can probe higher redshift. The reason is that, for the broad CMB kernel, spectroscopic redshift accuracy is not needed and the lower shot noise in DES play a mojor role in the delensing efficiency.
Even in the near future the CIB will still play the major role among LSS.
