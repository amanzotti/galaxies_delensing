...
Furthermore, by the mid 2020s, several next-generation LSS surveys will be online. Here, as an example for future optical galaxy surveys, we add LSST \cite{lsst-science-collaboration:2009} to the CMB lensing tracers. We test that other experiments like Euclid \cite{laureijs:2011} and WFIRST \cite{spergel:2013} have similar delensing performance. Even if they will observe similar part of the sky and similar modes, because of the different strengths and weaknesses of these experiments it will still be important to combine them efficiently.
We also consider a radio continuum survey modeled following the SKA planned experiment. Radio continuum observations of the 21 cm line are in theri early stage, and several experimental and data-analysis challenges need to be overcome. 
However, this technique has the potential to map LSS with relatively low noise up to redshift $z=6$. Here we considered just two possible  detection thresholds at 1 GHz (flux cut): 10 $\mu$Jy and 0.1 $\mu$Jy, which are representatives of the upper and lower limit levels for a SKA phase 1 (SKA1) and a SKA phase 2 (SKA2) survey.

The correlation factor with CMB lensing of stage 4 experiments is shown in \reffig{corrs4_10}  and in \reffig{corrs4_01} for SKA1 and SKA2 respectively.
...
With SKA and LSST, for the first we will have LSS tracers with a higher delensing efficiency than the CIB.
Even if galaxy survey will improve dramatically the CMB internal reconstruction will still be the main source of delensing. This, having a perfect kernel overlap with the true lensing potential will benefit from the very low level of noise of CMB S4 experiments.
...
CMB S4 delensing will probably be limited by small secondary effects like foregrounds contamination, missing modes due to filtering etc.
\reffig{corrs4_01} shows that a futuristic SKA-2 experiment will show a very high correlation with CMB lensing being more than 80\% correlated at all the scales. This performance is comparable with a CMB-S4 internal reconstruction.
