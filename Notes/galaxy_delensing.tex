...
These will translate into a better reconstruction of the B-modes in the measured patch, and, as a consequence will improve the constraints on inflation trough delensing. We model several actual and future surveys, and after computing the residual B-modes, we forecast the resulting statistical uncertainties on the amplitude and the shape of the inflationary tensor perturbations for current CMB experiments as well as the next generation (S3) and the planned fourth generation (S4). 
...
We will take into account the intensity mapping survey mode where SKA will measure the large-scale fluctuations of the \textit{integrated} 21cm intensity from many unresolved galaxies.  
...
However, given the low noise of this measurement and their small overlap with other probes they still improve the delensing efficiency even if their overlap with the CMB lensing potential is not optimal.
...
Of course, this is possible for galaxy surveys, but it is harder for a tracer with poor redshift information like the CIB or  radio continuum surveys. 
...
We used 10 and 4 photometric bins for LSST and DES respectively and we split DESI into 4 spectroscopic bins with no overlap among each others.
...
\caption{\textbf{Kernels Comparison with tomographic bins}: Comparison of the 10 LSST different kernels for every tomographic bin with the CMB lensing kernel. 
This will lead to a better delensing. }
\label{fig:kernel}
...
We assume a CMB experimental scenario composed of a high-resolution CMB experiment which defines the internal reconstruction performance, together with a low-noise, low-resolution experiment the B-modes of which will be delensed and used to constrain the inflationary parameters.
...
In the Fisher Information Matrix formalism \cite{10.2307/2342435}, the statistical uncertainty on a cosmological parameter $p$ can be obtain from the Fisher matrix $F_{ij}$ as $\sigma(p) =\sqrt{(\mathbf{F})_{pp}^{-1}}$.
...
After delensing the error can $\sigma^{\rm{del}}(p)$ can still be obtained from \refeq{fisher} with $C_{{l}}^{BB,\mathrm{res}}<C_{{l}}^{BB,\mathrm{lens}$ in the covariance defined in \refeq{cova}.
...
In \refeq{fisher} we are making two important assumptions.
Firstly we are fixing the all the other cosmological parameters. Uncertainties in those will however propagate to uncertainties in $\{r,n_{t}\}$. While the constraints will be slightly optimistic, this has no real impact on the estimate of the improvement due to delensing which is the main focus of this work.
Furthermore we are not considering the uncertainties on galaxy surveys internal parameters such as biases, source distributions and photometric redshift uncertainties. These parameters do have a significant impact in degrading inflationary constraints \cite{sherwin:2015,namikawa:2016a}.
However as shown in \cite{sherwin:2015,namikawa:2016a} these can be auto-calibrated i.e. thy can be tightly constrained using galaxy surveys auto spectra and cross correlation. 
...
Furthermore, we test how much not only CIB maps but also current low redshift galaxy surveys like DES and WISE can improve the delensing efficiency.
...
We thank S. Dodelson S. Passaglia K. Wu and K. Story for useful discussion and their review of the draft. Furthermore we thank D. Alonso for his feedback on radio continuum surveys.
