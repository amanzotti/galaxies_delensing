...
In this paper, we propose and study a possible way: using future galaxies surveys as tracers of the lensing potential in addition to other probes like SKA radio continuum survey, cosmic infrared background and internal CMB reconstruction. 
Furthermore, we point out how using redshift information through a clever tomographic binning can improve the delensing efficiency of galaxies survey.
...
We organize this article as follow: we describe the LSS tracers used in this analysis in \refsec{model}. In \refsec{th} we define the lensing B-mode component and the residual power after delensing with tracers of the lensing potential.The main results of this work is \refsec{for} : the improvement of inflationary parameters constraints due to delensing with CMB and LSS. We conclude in \refsec{concl}.
...
\end{split}
\end{equation}
Here $\frac{dN}{dz}$ is the number of galaxies observed by the survey as a function of redshift while $b(z)$ is the galaxy bias that connects the amplitude of galaxy overdensities to the one of the dark matter density underneath.
...
To compute the noise term, we assume that the available sky after masking is around $f_{sky}=0.44$ with 50 million galaxies \cite{ferraro:2015} and that the galaxies density is approximately uniform. Furthermore we adopt a liner bias $b_{\rm{WISE}}=1.41$ obtained by \cite{ferraro:2015} cross correlating WISE with Planck lensing potential.
...
Finally, we model both the redshift distribution and bias for the Radio Continuum (RC) survey conducted by the Square Kilometre Array (SKA) following \cite{namikawa:2016a}. 
...
Of course, this is possible for galaxies surveys, but it is harder for a tracer with poor redshift information like the CIB or  radio continuum surveys. 
...
We assume a CMB experimental scenario composed of a high-resolution CMB experiment which defines the internal reconstruction efficiency, together with a low-noise, low-resolution experiment the B-modes of which will be delensed and used to constrain the inflationary parameters.
...
We thank S. Dodelson K. Wu and K. Story for useful discussion. Furthermore we thank D. Alonso for his feedback on SKA-like surveys.
