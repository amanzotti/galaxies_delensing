...
With the improvement of the noise CMB internal reconstruction will become more important and the fraction of removed lensing B-modes will rapidly improve from the current level of Planck ($8\%$) and SPTPol ($35\%$) to 3G (56\%) and CMB S4 (76\%) level. \relook{multitracers} 
...
The improvement in cosmological parameters is summarized in \reftab{S3}. 
The main difference from the current generation of experiments is that in the near future CMB alone will be able to improve constraint on $r$ by a factor of 2 through internal delensing in the ideal case of no instrumental noise.

Despite this compared to CMB alone, galaxy surveys will still be able to remove an additional $20\%$ of power and to improve our constraint on the null hypothesis ($r=0$) by $15\%$. Note that apart from the addition of DESI, which plays a secondary role, the tracers used here are already available for the current generation. The ongoing effort can then allow this improvement even for the next generation of experiments. 
The combination of CMB and galaxy surveys will be able to improve the constrain on the null hypothesis of no primordial waves by a factor 1.8 or to improve the constraint of a possible detection (r=0.12) by 30\%.


\relook{something about $n_t$}
