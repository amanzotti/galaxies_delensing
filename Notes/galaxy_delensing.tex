...
\title{Future Cosmic Microwave Background delensing with galaxy surveys.}         % Enter your title between curly braces
...
We quantify this by the improvement due to delensing on the constraints on the inflationary tensor perturbations amplitude and shape ($r$ and $n_{t}$). We find that, in general, galaxy surveys should be splitted into tomographic bins as this can improve the correlation with CMB lensing by 30\%. 
Considering currently available survey, a DES-like galaxy survey can remove about $14\%$ of the lensing signal. The CMB experiments of the current generation will particularly benefit from large scale structure tracers that, once properly combined, will have a better performance than a CMB internal reconstruction.
With the decrease of the instrumental noise, the CMB internal reconstruction will increase its efficiency and the fraction of removed lensing B-modes with CMB alone will rapidly improve from the current level of Planck ($8\%$) and SPTPol ($35\%$) to 3G (56\%) and CMB S4 (85\%) level.  
Nevertheless optical galaxy surveys will still play an important role even for CMB S4. In particular LSST can a achieve a delensing performance comparable to a 3G CMB experiment but and completely different systematics. This will be important to prove the robustness against systematics of an eventual detection of primordial B-modes.
...
\end{equation}
%
where $\chi_*$ is the comoving distance to the last-scattering surface at $z_*\simeq 1090$, $\Omega_{\rm m}$ and $H_0$ are the present day values of the Hubble and matter density parameters, respectively. \relook{As can be seen in \reffig{kernel} the lensing kernel $W^{\kappa}(z)$ is quite a broad function of redshift peaking around $z=1.8$ but extending to high redshift.}
...
\relook{In \reffig{kernel}, we compare the kernels $W^{g}(z)$ of all the tracers introduced here to the CMB lensing kernel $W^{\kappa}(z)$.}
...
\relook{\reffig{kernel} shows how the CIB kernel peaks at higher redshift compared to other galaxy survey kernels, with a better overlap with the lensing one.}
...
\caption{Kernels Comparison: Comparison of the different kernels as a function of redshift for some of the tracers used in this analysis. The bigger the overlap with the CMB lensing kernel the better the reconstruction of the lensing potential will be leading to a higher delensing efficiency. }
\label{fig:kernel}
\end{center}
\end{figure}
% ===================
...
\caption{Kernels Comparison with tomographic bins: Comparison of the 10 LSST  tomographic bins together with the full LSST survey and the CMB lensing kernel. 
...
We assume a CMB experimental scenario composed of a high-resolution CMB experiment which defines the ability to internally reconstruct the potential $\kappa$, together with a low-noise, low-resolution experiment that measured the B-modes that will be delensed and used to constrain the inflationary parameters.
...
We define the error after delensing as $\sigma^{\rm{del}}(p)$.
...
We define the improvement as the ratio of the constraints before and after delensing: $\alpha_{r} = \sigma^{\rm{del}}(r)/\sigma(r)$ and $\alpha_{n_{t}} = \sigma^{\rm{del}}(n_{t})/\sigma(n_{t})$. The fiducial values used are described in \refsec{del_res}. Given a fiducial value for $r$, the value of $n_{t}$ is fixed imposing the consistency relation $n_t=-r/8$ \cite{LIDDLE1992391}.
...
Finally, although we have assumed a Gaussian covariance $\sigma( C_{\ell}^{BB} )$ it actually has small non-Gaussian contributions \cite{motloch:2017,benoit-levy:2012}. This approximation is good enough to show the improvement due to delensing.
...
First, we test the efficiency with an internal reconstruction of CMB lensing performed by the Planck collaboration \cite{planck-collaboration:2016a}. We compute the noise in the CMB lensing map in \refeq{noisekappa} using the noise curves publicly available
\footnote{\url{https://wiki.cosmos.esa.int/planckpla2015/index.php/Specially\_processed\_maps}}.
Then, we test a second scenario with a ground-based experiment with noise levels consistent with SPT-Pol: \textcolor{red}{$9.4\mu$K-arcmin} in polarization with a beam equal to \textcolor{red}{1.2 arcmin}. In this case we set the largest achievable scale at $\ell_{min}=300$. However, the results are robust against this choice since the neglected scales contribute only moderately to the reconstruction of the CMB lensing potential.
...
On the other end, the CMB lensing reconstruction from SPT-Pol will be more than 80\% correlated with the true field at $\ell<300$, and only then the correlation becomes comparable to the current LSS surveys.
...
For example, DES delensing efficiency is higher than WISE first because of the lower level of noise and also because DES galaxies are located at slightly higher redshift so they better overlap with CMB lensing. As expected the improvement of the internal CMB reconstruction reduces the relative importance of LSS. 
...
\caption{Correlation factor $\rho$ with the lensing potential $\kappa$ as a function of the angular scale $\ell$ for current generation tracers.
Here we show both current galaxy survey and internally reconstructed $\phi$ CMB lensing potential. 
...
For current surveys we only consider $p=r$ since the broad beam of the deep BK-like survey does not provide enough leverage to constrain the shape of the B-mode spectra. We still let $n_{t}$ to vary in our fisher calculation even if it is not very degenerate with $r$.
...
The lensing reconstruction noise in Planck is such that internal delensing can only remove $8\%$ of the power, improving the constraints on $r$ by a factor of 1.06 even in the ideal scenario of no instrumental noise in the B-modes. A similar performance is achieved with WISE. DES can do better, removing a level of 14\% of power. However, it covers a smaller fraction of the sky than the two previous probes. As shown in previous works \cite{sherwin:2015}, the best tracer is the CIB with a 27\% reduction in power.
Nonetheless, it is still worthwhile considering all the tracers. Indeed a multi tracer-approach can bring the removed power on the overlapping area from 27\% of the CIB alone to 36\% using all the LSS (LSS-S2) and 41\% when Planck lensing reconstruction is added to the set.
...
\reftab{current-spt} shows that, with an instrumental noise at the expected level for SPTPol, the CMB will soon be able to remove an amount of power (35\%) at the level of the CIB. Once combined with LSS tracer this will lead to removing 56\% of the power. 
The improvement of 1.57 in $\sigma(r)$ that will follow, even when realistic noise is considered, is such that it will make delensing a  needed step for CMB polarization data analysis.
...
LSS-S2 (36\%) &1.7& 1.29  & 1.09\\ \hline
CMB Planck (8\%) &1.06& 1.05  & 1.01 \\ \hline
LSS-S2 +CMB (41\%)& 1.77& 1.31& 1.10  \\ \hline
\end{tabular}
\label{tab:current}
\end{table}
...
We also test a scenario where primordial gravitational waves are present at their highest possible value of $r_{\rm{fid}}=0.12$ \cite{bicep2/keck-collaboration:2015}. As expected the importance of delensing itself is now reduced given that the lensing component constitutes a smaller portion of the total B-modes variance
 ...
 \begin{table}
\caption{$\alpha(r)$: Current generation improvements on $\sigma(r)$ due to delensing. We use the same LSS tracers as \reftab{current} but with the internal lensing reconstruction performed by SPTPol.}
  \vspace{0.2cm}
  \begin{tabular}{|c | c | c | c|}
\hline
Surveys &$\alpha_{r=0}, N^{B}_{\ell}=0$& $\alpha_{r=0}$ & $\alpha_{r=0.12}$ \\ \hline \hline
SPTPol (35\%) &1.46& 1.27  & 1.08 \\ \hline
LSS-S2 +SPTPol (56\%) & 2.18 & 1.57 & 1.16  \\ \hline
...
The two experiments combined will have a level of noise of \textcolor{red}{$3\mu$K-arcmin} and a \textcolor{red}{1 arcmin} beam. This level of noise is also assumed for the internal noise reconstruction.
With this experimental setup, we can push the angular scale range used to calculate the Fisher matrix to $50<\ell<3000$ even if most of the high $\ell$ scale do not contribute to the constraints.
Not only will CMB experiments improve shortly: eventually DESI will start taking data. For this reason, we add DESI to the LSS tracers used in \refsec{del_res}. 

The correlation factor attainable using generation 3 experiments is shown in \reffig{corrS3}.
An interesting finding is that DESI will be less efficient (removing 10\% of the power) than a DES-like survey (14\%) despite the fact that it can probe slightly higher redshift. The reason is that, because of the broad CMB kernel, spectroscopic redshift accuracy is not needed and the lower shot noise in DES increases the delensing efficiency significantly. Unfortunately, adding DESI will only bring the power removed using LSS from 36 to 41\%.
...
\caption{Correlation factor $\rho$ with the lensing potential $\kappa$ as a function of the angular scale $\ell$. This figure is the same as \reffig{corrnow} but with a stage-3 CMB experiment performing the internal reconstrcution and with DESI added to the stage-2 galaxy surveys (LSS-S2). }
\label{fig:corrS3}
\end{center}
\end{figure}
...
An ambitious program for a stage-4 ground CMB experiment is currently under planning \cite{abazajian:2016}. 
Moreover, satellite and balloon CMB experiments have been proposed and have the potential to extend the accessible B-mode measurement to the largest scales.
Following \cite{abazajian:2016} we assume a CMB-S4 ground experiment with a level of noise of \textcolor{red}{$1\mu$K-arcmin} and a \textcolor{red}{1 arcmin} beam.
Furthermore, by the mid-2020s, several next-generation LSS surveys will be online. Here, as an example for future optical galaxy surveys, we add LSST \cite{lsst-science-collaboration:2009} to the CMB lensing tracers. 
We test that other experiments like Euclid \cite{laureijs:2011} and WFIRST \cite{spergel:2013} have similar delensing performance. 
Even if they will observe an overlapping part of the sky and similar modes, because of their different strengths and weaknesses it will still be important to combine them efficiently.
...
However, this technique has the potential to map the LSS with relatively low noise up to redshift $z=6$. Here we considered just two possible  detection thresholds at 1 GHz (flux cut): 10 $\mu$Jy and 0.1 $\mu$Jy, which are representatives of the upper and lower limit levels for a SKA phase 1 (SKA1) and a SKA phase 2 (SKA2) survey.

The correlation factor with CMB lensing for stage-4 experiments is shown in \reffig{corrs4_10}  and in \reffig{corrs4_01} for SKA1 and SKA2 respectively.
...
Despite the improvement in galaxy surveys, the CMB internal reconstruction will still be the main source of delensing. This, having a perfect kernel overlap with the true lensing potential will benefit from the very low level of noise of CMB S4 experiments.
...
At this level, CMB S4 internal delensing will probably be limited by small secondary effects like foregrounds contamination, missing modes due to filtering etc.
Finally, \reffig{corrs4_01} shows that a futuristic SKA-2 experiment will have a very high correlation with CMB lensing, being more than 80\% correlated at all the scales. This performance is comparable to a CMB-S4 internal reconstruction.


\reftab{S4} shows the improvement on the inflationary constraint given the cross-correlation factors described above. 
LSST alone will be able to remove almost half of the B-mode lensing power. This will lead to an improvement in $\sigma(r)$ of a factor of 1.8. If we add all the other LSS tracers previously included (i.e. excluding SKA) the improvement factor increase to 2.3. 
SKA in its phase 1 will remove 53\% of the lensing power increasing 
...
\hline
Surveys & $\alpha_{r=0}, N^{B}_{\ell}=0$ & $\alpha_{r=0}$ & $\alpha_{r=0.12}$ \\ \hline \hline
LSST (48\%) &1.88 & 1.81  &  \\ \hline
LSS  (60\%) &2.47 & 2.31  & \\ \hline
SKA (10$\mu$Jy) (53\%) &2.18& 2.06  & \\ \hline
SKA (0.1$\mu$Jy) (77\%) &4.6& 3.96  & \\ \hline
LSS + SKA (10$\mu$Jy)  (62\%) &2.74 & 2.53  &  \\ \hline
LSS + SKA (0.1$\mu$Jy)  (80\%) &4.66 & 3.96  &  \\ \hline
CMB S4 (81\%) & 4.63 & 3.96  &  \\ \hline
LSS+CMB (69\%)& 2.8 & 1.8&  \\ \hline
\end{tabular}
\label{tab:S4}
\end{table}
...
SKA (0.1$\mu$Jy) (40\%) &1.73& 1.42  & 1.2\\ \hline
...
For ongoing experiments, we find that combining LSS tracers with CMB internal reconstruction will be particularly beneficial.
An optical survey like DES alone is able to remove $14\%$ of the lensing component and together with WISE($8\%$) and the CIB ($27\%$) will allow to remove $36\%$ of the contaminant signal using only LSS survey.
...
In the future, the decreasing level of instrumental noise of CMB experiments will dramatically improve the internal reconstruction of the structures lensing the CMB. 
The fraction of removed lensing B-modes will rapidly improve from the current level of Planck ($8\%$) and SPTPol ($35\%$) to 3G (56\%) and CMB S4 (85\%) level. 
...
For example, a tomographically binned LSST-survey will remove 56\% of the lensing power. While this performance is comparable to a Stage-3 CMB it will allow to probe the robustness against systematics of an eventual detection.
Indeed delensing with just CMB data will require a careful study of possible biases and systematics effects due to the use of the same source (the CMB) we are delensing to reconstruct the lensing effect itself \cite{carron:2017,sehgal:2016,namikawa:2017}.
...
For example, LSS delensing allows to selectively remove from the CMB  the gravitational effect coming from the non-linear structures in the low-redshift universe. 
...
We thank S. Passaglia, S. Dodelson, K. Story and K. Wu for useful discussion and their review of the draft. 
