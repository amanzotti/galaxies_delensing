...
Here we discuss the role of current and future large scale structure surveys in a multi-tracers approach to delensing that will improve thesreconstruction of the lensing potential that lenses the CMB photons and, as a consequence, the delensing efficiency.
...
Among currently available surveys, a DES-like galaxy survey can remove about $17\%$ of the lensing signal. Ongoing or recently completed CMB experiments (CMB-S2) will particularly benefit from large scale structure tracers that, once properly combined, will have a better performance than a CMB internal reconstruction.
With the decrease of instrumental noise, the CMB internal reconstruction will increase its efficiency and the fraction of removed lensing B-modes with CMB alone will rapidly improve from the current level of Planck ($7\%$) and SPTPol ($31\%$) to 3G (56\%) and CMB S4 (81\%) level.  
...
\caption{Kernel Comparison: Comparison of the different kernels as a function of redshift for some of the tracers used in this analysis. The bigger the overlap with the CMB lensing kernel the better the reconstruction of the lensing potential is thus leading to a higher delensing efficiency. Note that for galaxy surveys the kernel corresponds to $b(z)\frac{dN}{dz}$ and not simply to the redshift distribution of galaxies.}
\label{fig:kernel}
\end{center}
\end{figure}
% ===================
...
The conclusions about the relative importance of LSS tracers we find for $\sigma(r)$ are still valid for $\sigma(n_{t})$. Delensing can then be useful also to constrain more exotic inflationary models.
