...
The bigger the $\rho_{\ell}^{2}$ is for an LSS field, the more it is correlated with the lensing potential acting on the CMB photons. A higher correlation allows for a better reconstruction of $\phi$ and, as a consequence, of $B^{\rm{lens}}$ leading to a smaller residual power $C_{{l}}^{BB,\mathrm{res}}$.
...
We start by assuming that we have n different tracers of the gravitational potentials $I_{i}$ with $i\in \{1,..,n\}$. We can optimally combine them to estimate $\phi$ or, in other word, to maximize the correlation factor $\rho$ with:
\beqn
I         &=&    \sum_{i}c^{i}I^{i} \nonumber \\
...
CIB and 21 cm surveys probe the high redshift structures and, independently from the model assumed, they show a relatively good overlap with the CMB lensing kernel. 
...
The parameter uncertainties are inversely proportional to the covariance of the measurement. Since the lensing B-modes $C_{{l}}^{BB,\mathrm{lens}}$ are a substantial component of the covariance, removing part of them through delensing will reduce the parameter statistical error.
...
Finally, we have assumed a Gaussian covariance $\sigma( C_{\ell}^{BB} )$ even if has small non-Gaussian contributions \cite{motloch:2017,benoit-levy:2012}. This approximation is good enough to show the improvement due to delensing.
...
Finally, \reffig{corrs4_01} shows that a futuristic SKA-2 experiment will have a very high correlation with CMB lensing, being more than 80\% correlated at all the scales. This performance is comparable with a CMB-S4 internal reconstruction.
