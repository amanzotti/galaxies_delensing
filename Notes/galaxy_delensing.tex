...
Even if there is not a definite distinction between different stages, we focus on three distinct scenarios: the ongoing or recently concluded stage (S2), a third generation stage (3G) of experiments in an advanced building phase or that have just started taking data and finally the futuristic CMB Stage 4.
...
We assume that the CMB lensing reconstruction is performed by the higher resolution CMB experiment. We explore two possibilities.
...
On the other end, the CMB lensing reconstruction from SPT-Pol will be more than \textcolor{blue}{70\%} correlated with the true field at $\ell<300$, and only then the correlation goes below the level of current LSS surveys.
...
The lensing reconstruction noise in Planck is such that internal delensing can only remove $7\%$ of the power, improving the constraints on $r$ by a factor of 1.06 even in the ideal scenario of no instrumental noise in the B-modes. A slightly better performance is achieved with WISE. DES can do better, once binned in redshift, removing a level of $17\%$ of power. However, it covers a smaller fraction of the sky than the two previous probes. As shown in previous works \cite{sherwin:2015}, the best tracer is the CIB with a $30\%$ reduction in power.
Nonetheless, it is worthwhile combining all the tracers. Indeed a multi tracer-approach can bring the removed power on the overlapping area from $30\%$ of the CIB alone to $43\%$ using all the LSS (LSS-S2) and $45\%$ when Planck lensing reconstruction is added to the set.
...
Given the improvement in the noise of the high-resolution experiment, CMB alone will be able to improve constraint on $r$ by a factor of $\sim 2.4$ through internal delensing in the ideal case of no instrumental noise in the B-modes (but with instrumental noise in the E-mode used to delens).
Adding galaxy surveys will lead to a further improvement without additional effort.
Indeed galaxy surveys will still be able to remove an additional $11\%$ of power from $C_{\ell}^{BB,\mathrm{lens}}$ and to improve our constraint on the null hypothesis ($r=0$) by $34\%$. Note that apart from the addition of DESI, the tracers used here are already available today. The ongoing effort can then lead to a significant improvement even for the next generation of experiments. 
Finally, the combination of CMB and galaxy surveys will be able to improve the constrain on the null hypothesis of no primordial waves by a factor \textcolor{blue}{1.8} or to improve the constraint of a possible detection (r=0.07) by 34\%.
...
\hline
Surveys & $\alpha_{r=0}, N^{B}_{\ell}=0$ & $\alpha_{r=0}$ & $\alpha_{r=0.07}$ \\ \hline \hline
DESI (11\%) &1.13 & 1.08  & 1.03 \\ \hline
LSS-S3 (45\%) &1.82& 1.59  & 1.18\\ \hline
CMB 3G (56\%) &2.31& 1.65  & 1.20 \\ \hline
LSS-S3+CMB (68\%)& 3.2 & 2.3& 1.21  \\ \hline
\end{tabular}
\label{tab:S3}
\end{table}
...
LSST alone will be able to remove almost half of the B-mode lensing power. This will lead to an improvement in $\sigma(r)$ of a factor of 2. If we add all the other infrared and optical LSS tracers previously included (i.e. excluding SKA) the improvement factor increase to 2.5.
...
Finally, as previously mentioned, CMB S4 will be the main source of delensing with the ability of removing up to more than 80\% of the B-modes power. This will bring an almost factor of 3.7 improvement in the null test case even with realistic noise. Combining with LSS will only increase the removed power compared to CMB only by $\simeq 4.3\%$ but it will be highly beneficial to test the robustness of the final result.
...
\hline
Surveys & $\alpha_{r=0}, N^{B}_{\ell}=0$ & $\alpha_{r=0}$ & $\alpha_{r=0.07}$ \\ \hline \hline
LSST (51\%) &2.0 & 1.9  & 1.21  \\ \hline
LSS-S4  (59\%) &2.4 & 2.1  & 1.25 \\ \hline
SKA (10$\mu$Jy) (53\%) &2.1& 1.9  & 1.22\\ \hline
LSS-S4 + SKA (10$\mu$Jy)  (62\%) &2.6 & 2.3  & 2.5  \\ \hline
CMB S4 (81\%) & 5.0 & 3.7  & 1.43 \\ \hline
LSS-S4+CMB (86\%)& 6.6 & 4.3  &  \\ \hline
\end{tabular}
\label{tab:S4}
\end{table}
...
We focus on galaxy clustering here, leaving other probes like weak lensing, for future works. However,  weak lensing is usually lower redshift probes than clustering resulting in a worse delensing efficiency. 
