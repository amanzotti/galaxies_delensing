...
Given a 2D projected field $\delta^{i}$ this can be seen as a 3D field project along the line of site as:

\begin{equation}
\delta^{i}(\theta) = \int_0^{\infty} dz\, W^{\kappa}(z)\delta^{i}(\chi(z)\nver,z).
\label{eqn:wkappa}
\end{equation}
...
This lensing potential can be reconstructed from the CMB. In that case it can be see as a noisy tracer of the true field. While the CMB internal reconstruction fiels as the same kernel as the true one $W^{\kappa}(z)$.
Given certain instrumental noise level and beam, results in a lensing map with reconstruction noise $N_l^{\kappa \kappa}$, so that its power spectrum is
\beq
...
Different galaxy surveys in this work are fully characterized by their b(z) and $\frac{dN}{dz}$.
...
We test the delensing efficiency modelling  current survey after D.E.S and using as future probe galaxy clustering measured with D.E.S.I and LSST together with 21 cm measurement like SKA.
