...
\caption{Correlation factor between galaxies survey and CMB lensing potential as a function of the multipole $\ell$.}
\label{fig:corr}
\end{center}
\end{figure}
%=======================================
...
\subsection{Galaxies contribution at different redshift}\label{sec:galcontrib}

Let's now assume that we have n different tracers of the gravitational potentials $I_{i}$ with $i\in \{1,..,n\}$. It can be shown that the optimal way to combine them to obtain the optimal estimator of $\phi$ or, in other word maximizing the correlation factor $\rho$ is:
\beqn
I 		&=&	\sum_{i}c^{i}I^{i} \nonumber \\
