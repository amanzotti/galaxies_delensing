...
The amplitude of this primordial B-mode component is parameterized by the ratio of the amplitudes of the primordial tensor and scalar perturbations, $r$. The shape of this component, i.e. the amplitude as a function of the angular scales is encoded in the tensor tilt $n_{t}$.
...
Measuring or constraint the value of $r$ and $n_t$ will put a tight constraint on the allowed models of inflation.
...
Unfortunately, the observed B modes are not solely due to primordial gravitational waves but also by late time effects. In this work, we will focus on gravitational interactions with LSS.
...
Indeed, current experiments are already limited by the contamination from lensing B modes since the instrument noise is below the lensing B-mode $rms$ fluctuations \citep{bk14}.
...
In this paper, we study the possible role of future galaxies surveys as tracers of the lensing potential. These will translate into a better reconstruction of the B-modes in the measured patch and as a consequence improve the constraints on inflation trough delensing. We model several actual and future surveys, and after computing the residual B-modes, we forecast the statistical uncertainness on $r$ and $n_T$ for current CMB experiments as well as the next generation (S3) and the planned fourth generation (S4).

This article is organized as follow: in \refsec{th} we describe the lensing B-mode components and the residual power after delensing with tracers of the lensing potential. We describe the LSS tracers used in this analysis in \refsec{model}. \refsec{for} is the main results of this work: the improvement of inflationary parameters constraints due to delensing with CMB and LSS. We conclude in \refsec{concl}.
...
Here we discuss the improvement due to combining these and other available probes (see \cite{yu:2017} for publicly accessible multi-tracers data-products).
...
CMB measurements are rapidly improving, and the next generation of ground-based telescopes has been already deployed, and data are currently being taken.
For the CMB at the beginning of the next decade, we will assume a deep experiment with a level of noise of \textcolor{red}{$3\mu$K-arcmin} with a \textcolor{red}{1 arcmin} beam. This is a reasonable level for a combination of the next generation of BK and SPT3G. This level of noise will also be assumed for the internal noise reconstruction.

The correlation attainable using generation 3 experiments is shown in \reffig{corrS3}.
Not only will CMB experiments improved shortly: possibly D.E.S.I will start taking data. For this reason, we add DESI to the previously mentioned LSS tracers. 
...
In this very futuristic but reasonable scenario, we add to the available large scale structures both 21 cm surveys like SKA and large scale structures like LSST and Euclid.
