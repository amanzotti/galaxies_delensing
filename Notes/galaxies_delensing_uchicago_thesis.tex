...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=1.]{../images/BB_res_ell2_thesis.pdf}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=1.]{../images/compare_kernel_thesis.pdf}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=1.]{../images/compare_kernel_tomo_thesis.pdf}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=1.]{../images/B_res_bin_thesis.pdf}
...
First, we test the delensing efficiency with an internal reconstruction of CMB lensing performed by the Planck collaboration \cite{planck-collaboration:2016a}. We compute the noise in the CMB lensing map in \refeq{noisekappa} using the noise curves publicly available
\footnote{https://wiki.cosmos.esa.int/planckpla2015/index.php/Specially\_processed\_maps}.
Then, we test a second scenario with a ground-based experiment characterized by noise levels consistent with SPT-Pol: $9.4\mu$K-arcmin in polarization with a beam equal to 1.2 arcmin. In this case we set the largest achievable scale at $\ell_{min}=300$. However, the results are robust against this choice since the neglected scales contribute only moderately to the reconstruction of the CMB lensing potential.
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=1.]{../images/actual_scenario_planck_thesis.pdf}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=1.]{../images/actual_scenario_thesis.pdf}
\caption{Correlation factor $\rho$ with the CMB lensing potential as a function of the angular scale $\ell$ for completed and ongoing Stage-2 tracers. 
...
Nonetheless, it is worthwhile combining all the tracers. Indeed a multi tracer-approach can bring the removed power on the overlapping area from 27\% of the CIB alone to 36\% using all the LSS (LSS-S2) and 41\% when Planck lensing reconstruction is added to the set.
...
The values in the other columns correspond to ratio of the error before and after delensing for 3 cases: no instrumental noise in the B-mode measurement and realistic noise for two different values of $r$.}
  \vspace{0.2cm}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=1.]{../images/S3_scenario_thesis.pdf}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=1.]{../images/S4_scenario_ska10_thesis.pdf}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=1.]{../images/S4_scenario_ska01_thesis.pdf}
\caption{Correlation factor $\rho$ with the CMB lensing potential as a function of the angular scale $\ell$. This figure is the same as \reffig{corrnow} but with a stage-4 CMB experiment performing the internal reconstrcution and with LSST and SKA added to the stage-2 and stage-3 galaxy surveys (LSS-S3). }
\label{fig:corrs4_01}
\end{center}
\end{figure}
...
Depending on the CMB instrumental noise and the amount of galactic foreground cleaning, delensing using LSS survey can correspond to a maximum improvement of 30\% in the constraint on the tensor to scalar ratio $r$ compared to the value before delensing.
...
\begin{figure}[htbp]
%\begin{center}
\includegraphics[scale=0.5]{../images/errors_summary_thesis_noted.pdf}
\caption{Summary of the amount of lensing B-modes power removed by different tracers for different generations of experiments.
The bars on the left represent the individual contribution of different galaxy tracers. On the right, the gray bar represents the contribution from LSS only and the yellow one the total removed power once LSS and CMB internal reconstruction are combined.
The values are taken from \reftab{current}, \reftab{current-spt}, \reftab{S3} and \reftab{S4}. }
\label{fig}
%\end{center}
\end{figure}
% ===================
