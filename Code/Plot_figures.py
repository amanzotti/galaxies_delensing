...
plt.plot(
    BB_contr[:, 0],
    BB_contr[:, 1] - np.min(BB_contr[:, 1]),
    '--',
    alpha=0.6,
    linewidth=font_size / 14.,
    color='k')
...
plt.savefig(
    '../Notes/images/actual_scenario_planck.pdf', dpi=600, papertype='Letter')
...
plt.plot(
    BB_contr[:, 0],
    BB_contr[:, 1] - np.min(BB_contr[:, 1]) + 0.45,
    '--',
    alpha=0.6,
    linewidth=font_size / 14.,
    color='k')
...
plt.savefig(
    '../Notes/images/S4_scenario_ska10.pdf', dpi=600, papertype='Letter')
...
rho_names = [
    'test', 'rho_cib.txt', 'rho_des.txt', 'rho_gals.txt', 'rho_wise.txt',
...
rho_names = [
    ...
    'rho_cmb_S3.txt', 'rho_gals_S3.txt', 'rho_comb_S3.txt', 'rho_cmb_S4.txt',
    'rho_gals_S4.txt', 'rho_comb_S4.txt'
]
...
plt_func(
    clbb_tens(r=0.01, lmax=3000),
    label=r'$C^{BB^{\rm{tens}}}_{\ell} ~ _{r=0.01}$',
    linestyle='--')

plt_func(
    clbb(r=0.0, lmax=3000),
    label=r'$C^{BB^{\rm{lens}}}_{\ell}$',
    linewidth=font_size / 12.5,
    alpha=0.8)

plt_func(
    ell,
    ell * (ell + 1) * np.array(B_res3[1]) / 2. / np.pi,
    label=r'$C^{BB^{\rm{res}}}_{\ell} (\rm{CIB})$',
    linewidth=font_size / 12.5,
    alpha=0.8)
plt_func(
    ell,
    ell * (ell + 1) * np.array(B_res3[-1]) / 2. / np.pi,
    label=r'$C^{BB^{\rm{res}}}_{\ell} (\rm{S4})$',
    linewidth=font_size / 12.5,
    alpha=0.8)
