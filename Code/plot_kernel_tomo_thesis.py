...
for i, bin_tomo in enumerate(lsst_tomo_bins):
    plt.plot(z, bin_tomo.dndzfun(z) / np.max(lsst.dndzfun(z)) *
             galaxies_fraction_lsst[i], color=colors[i], alpha=0.8)

plt.plot(z, lsst.dndzfun(z) / np.max(lsst.dndzfun(z)), alpha=0.8, color='#1f77b4', label='LSST')
...
plt.plot(zs, w_kappa / np.max(w_kappa), alpha=0.8, color='#d62728', label='CMB')
...
# ============================================
# FINALLY SAVE
plt.savefig('../images/' + 'compare_kernel_tomo_thesis.pdf', papertype='Letter')
plt.savefig('../images/' + 'compare_kernel_tomo_thesis.png', papertype='Letter')
plt.clf()
