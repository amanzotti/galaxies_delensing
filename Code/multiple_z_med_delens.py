...
def compute_rho_sigle_tracer(surveys,
                             lbins,
                             cls,
                             ckk,
                             output_dir,
                             galaxies_steradians=1e15):
    cl_cross_k = {}
    cl_auto = {}
    rho = {}
    ...
    for label in surveys:
        ...
        np.savetxt(output_dir + '/limber_spectra/rho_test_' + label +
                   '_ngal_{:.1f}.txt'.format(galaxies_steradians),
                   np.vstack((lbins, rho[label])).T)
    return rho


def compute_rho_single_tracer_plus_CMB(surveys,
                                       lbins,
                                       cls,
                                       ckk,
                                       ckk_noise,
